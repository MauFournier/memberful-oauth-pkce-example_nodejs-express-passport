/****************************************************************
 ** Memberful OAuth2 PKCE Example - Node.js + Express + Passport
 **
 ...
 **
 ** This is the flow you would use for a client-side single-page application
 ** or a mobile app. If you're building a server-side
...
app.get('/', (req, res) => {
  res.send(`
  <html><head></head><body>
    <h1>Memberful OAuth PKCE Example - NodeJS + Express (with Passport auth library)</h1>
...
const strategy = new OAuth2Strategy(
  {
    ...
    // Since we're using PKCE here (this example isn't for a server-side app),
    // we'll include the `state` and `pkce` parameters, to signal to Passport
    // that we're using a PKCE flow. For more details about PKCE, see:
    // https://memberful.com/help/custom-development-and-api/sign-in-for-apps-via-oauth/
    state: true,
    pkce: true,
  },
